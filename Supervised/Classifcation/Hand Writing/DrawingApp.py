...
class DrawingApp:
    ...
    def detect(self):
        ...
        predict_tf,confidence_tf=self.detector.predict_tf(self.image)
        self.info.config(text=f"predict-tf : {predict_tf} - {confidence_tf:.2f}% \npredict : {predict} - {confidence:.2f}%")
